...
the name of the sequence (located on the line above the sequence line).

2) Search for the same sequence in both fastq files.
...
scrolls by on the terminal and then runs through another command.
...
With `grep`, `wc` and a little dividing we can find out how many sequences we're finding.
...
That's only the first line but it is a lot to take in `cut` is a program that will extract columns in tab-delimited files.  It is a very good command to know.  Lets look at just the first four columns in the header using the `|` readirect and `cut`
